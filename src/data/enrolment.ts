...
type EnrolmentStep = { id: string; label: string; fields: readonly FieldPath<EnrolmentFormValues>[] };
...
export const durationOptions = [1,2,3,6,12].map((months) => ({ label: `${months} ${months === 1 ? "month" : "months"}`, value: String(months) }));
