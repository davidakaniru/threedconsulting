...
export function EnrolmentSummary({ values, programmeLabel }: Props) {
  ...
  return <div className="rounded-3xl bg-primary/4 px-5 py-2"><dl>
    ...
    <Row label="Child" value={`${values.childFirstName} ${values.childLastName}`} /><Row label="Date of birth" value={values.childDateOfBirth} /><Row label="Current class / education level" value={values.currentEducationLevel} />
