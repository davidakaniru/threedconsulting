...
export function EnrolmentSummary({ values, programmeLabels = [] }: Props) {
  ...
  return (
    <div className="rounded-3xl bg-primary/4 px-5 py-2">
      <dl>
        {!values.hasParentAccount && (
          <>
            <Row
              label="Parent"
              value={`${values.parentFirstName ?? ""} ${values.parentLastName ?? ""}`}
            />
        ...
        <Row
          label="Child"
          value={`${values.childFirstName} ${values.childLastName}`}
        />
        <Row label="Date of birth" value={values.childDateOfBirth} />
        <Row
        ...
        <Row
          label="Duration"
          value={`${values.durationMonths} ${values.durationMonths === 1 ? "month" : "months"}`}
        />
        {values.additionalMessage && (
          <Row label="Additional message" value={values.additionalMessage} />
        )}
      </dl>
    </div>
  );
