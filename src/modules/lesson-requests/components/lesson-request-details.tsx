...
export function LessonRequestDetails({
  request,
  assignment,
}: {
  request: LessonRequestDetail;
  assignment?: LessonAssignmentView | null;
}) {
  return (
    <div className="space-y-6">
      ...
      >
        <dl>
          ...
          <Row
            label="Date of birth"
            value={new Date(request.childDateOfBirth).toLocaleDateString(
              "en-GB",
            )}
          />
          <Row
          ...
          <Row
            ...
            value={formatTime(request.preferredTime)}
          />
          <Row
            label="Lesson duration"
            value={`${request.durationMonths} ${request.durationMonths === 1 ? "month" : "months"}`}
          />
