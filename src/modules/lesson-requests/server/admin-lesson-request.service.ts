...
function displayName(profile: any) {
  return [profile?.first_name, profile?.last_name].filter(Boolean).join(" ") || profile?.email || "Parent";
}
...
async function parentProfiles(parentIds: string[]) {
  ...
  if (error) throw new ApiError("LESSON_REQUEST_PARENTS_LOAD_FAILED", "Parent details could not be loaded.", 500);
...
export async function listLessonRequests(params: Record<string, unknown>): Promise<LessonRequestListResult> {
  ...
  if (params.search) {
    ...
    if (search) query = query.or(`child_first_name.ilike.%${search}%,child_last_name.ilike.%${search}%`);
  }

  const { data, error, count } = await query;
  if (error) throw new ApiError("LESSON_REQUESTS_LOAD_FAILED", "Enrolments could not be loaded.", 500);
  const profiles = await parentProfiles((data ?? []).map((row: any) => row.parent_id));
  const requests = (data ?? []).map((row: any) => mapSummary(row, profiles.get(row.parent_id)));

  return { requests, total: count ?? requests.length, page, pageSize };
}

export async function getLessonRequest(id: string): Promise<LessonRequestDetail> {
  const { data, error } = await db()
    .from("lesson_requests")
    .select("*,programmes(id,name,slug)")
    .eq("id", id)
    .maybeSingle();
  if (error) throw new ApiError("LESSON_REQUEST_LOAD_FAILED", "The enrolment could not be loaded.", 500);
  if (!data) throw new ApiError("LESSON_REQUEST_NOT_FOUND", "Enrolment not found.", 404);
...
export async function getLessonRequestMetrics(): Promise<LessonRequestMetricsI> {
  ...
  const results = await Promise.all(statuses.map((status) => {
    let query = db().from("lesson_requests").select("id", { count: "exact", head: true });
    if (status) query = query.eq("status", status);
    return query;
  }));
  if (results.some((result: any) => result.error)) throw new ApiError("LESSON_REQUEST_METRICS_FAILED", "Enrolment metrics could not be loaded.", 500);
  return {
...
export async function publishLessonRequest(id: string, actorId: string) {
  ...
  if (current.status !== "pending_review") {
    throw new ApiError("LESSON_REQUEST_NOT_PUBLISHABLE", "Only enrolments awaiting review can be published.", 409);
  }
  const now = new Date().toISOString();
  const { data, error } = await client
    .from("lesson_requests")
    .update({ status: "open", reviewed_by: actorId, reviewed_at: now, published_at: now })
    .eq("id", id)
    .eq("status", "pending_review")
    .select("id,status,published_at")
    .maybeSingle();
  if (error) throw new ApiError("LESSON_REQUEST_PUBLISH_FAILED", "The enrolment could not be published.", 500);
  if (!data) throw new ApiError("LESSON_REQUEST_ALREADY_CHANGED", "This enrolment has already changed. Refresh and try again.", 409);
  await writeAuditLog({
    actorId,
