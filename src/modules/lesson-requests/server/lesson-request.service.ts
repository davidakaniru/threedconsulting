...
async function currentParentId() {
  ...
  const { data: profile } = await supabase.from("profiles").select("id,role,status").eq("id", data.user.id).maybeSingle();
  if (!profile || profile.role !== "parent" || profile.status !== "active") throw new ApiError("PARENT_ACCOUNT_REQUIRED", "Please use a parent account to enrol a child.", 403);
  return profile.id;
}

export async function submitUnifiedLessonRequest(input: SubmitLessonRequest, origin: string) {
  ...
  if (!parentId) {
    if (!input.email || !input.password || !input.parentFirstName || !input.parentLastName || !input.phone) throw new ApiError("ACCOUNT_DETAILS_REQUIRED", "Please complete your parent account details.", 422);
    const supabase = await createClient();
    const { data, error } = await supabase.auth.signUp({
      email: input.email.trim().toLowerCase(), password: input.password,
      options: { emailRedirectTo: `${origin}/portal/parent`, data: { first_name: input.parentFirstName.trim(), last_name: input.parentLastName.trim() } },
    });
    if (error || !data.user) throw new ApiError("ACCOUNT_CREATE_FAILED", error?.message ?? "Your parent account could not be created.", error?.status === 422 ? 409 : 500);
    if (Array.isArray(data.user.identities) && data.user.identities.length === 0) throw new ApiError("ACCOUNT_EXISTS", "An account already exists with this email. Please sign in to enrol another child.", 409);
    parentId = data.user.id;
    createdUserId = data.user.id;
    requiresEmailConfirmation = data.session === null;
    await (admin as any).from("profiles").update({ phone: input.phone.trim() }).eq("id", parentId);
  }

  const uniqueDays = [...new Set(input.preferredDays)];
  const { data: request, error } = await (admin as any).from("lesson_requests").insert({
    parent_id: parentId,
  ...
  if (error || !request) {
    ...
    throw new ApiError("LESSON_REQUEST_CREATE_FAILED", "Your lesson request could not be submitted. Please try again.", 500);
  }
