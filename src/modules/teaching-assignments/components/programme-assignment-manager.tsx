...
export function ProgrammeAssignmentManager({ programmeId, teachers }: { programmeId: string; teachers: TeacherOption[] }) {
  ...
  return <SectionCard title="Teaching assignments" description="Assign active teachers who are allowed to teach this programme." contentClassName="p-5 sm:p-6">
    ...
    {query.isLoading ? <p className="text-sm text-slate-500">Loading assignments…</p> : query.data?.assignments.length ? <div className="space-y-3">{query.data.assignments.map((assignment) => {
      ...
      return <div key={assignment.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between"><div><Link href={`/portal/admin/teachers/${assignment.teacherId}`} className="font-bold text-slate-900 hover:text-primary">{name}</Link><p className="mt-1 text-xs text-slate-500">{assignment.teacher.employeeId} · {assignment.teacher.email}</p><div className="mt-2"><StatusBadge status={assignment.status}/></div></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => update.mutate({ id: assignment.id, values: { status: assignment.status === "active" ? "inactive" : "active", primaryInstructor: assignment.primaryInstructor } })}>{assignment.status === "active" ? "Deactivate" : "Activate"}</Button><Button variant="ghost" size="icon-sm" aria-label="Remove assignment" onClick={() => remove.mutate(assignment.id)}><Trash2/></Button></div></div>})}</div> : <EmptyState icon={GraduationCap} title="No teachers assigned" description="Assign the first teacher to make this programme available for cohort creation."/>}
  </SectionCard>;
}
