...
export function ProgrammeAssignmentManager({
  programmeId,
  teachers,
}: {
  programmeId: string;
  ...
  const assigned = new Set(
    (query.data?.assignments ?? []).map((item) => item.teacherId),
  );
  const options = teachers
    .filter((teacher) => !assigned.has(teacher.id))
    .map((teacher) => ({
      value: teacher.id,
      label: `${[teacher.firstName, teacher.lastName].filter(Boolean).join(" ") || teacher.email} · ${teacher.employeeId}`,
    }));
          ...
          {query.data.assignments.map((assignment) => {
            ...
            return (
              ...
              >
                <div>
                  <Link
                    href={`/portal/admin/teachers/${assignment.teacherId}`}
                    className="font-bold text-slate-900 hover:text-primary"
                  >
                    {name}
                  </Link>
                  <p className="mt-1 text-xs text-slate-500">
                    {assignment.teacher.employeeId} · {assignment.teacher.email}
                  </p>
                  <div className="mt-2">
                        ...
                        values: {
                          status:
                            assignment.status === "active"
                              ? "inactive"
                              : "active",
                          primaryInstructor: assignment.primaryInstructor,
                        },
                      })
                    }
                  >
