...
import { SectionCard, StatusBadge, InfoCard } from "@/components/admin/ui";
import { useParentAction } from "@/modules/parents/hooks";
import type { ParentDetail } from "@/modules/parents/types";
        ...
        <SectionCard contentClassName="p-6">
          <div className="flex justify-between gap-4">
            ...
            <Button asChild variant="outline">
              <Link href={`/portal/admin/parents/${parent.id}/edit`}>
                <Pencil />
                Edit
              </Link>
            </Button>
          ...
          {currentParent.students.length ? (
            currentParent.students.map((s) => (
              <Link
                ...
                className="flex justify-between rounded-xl border p-4 hover:bg-muted"
              >
                <div>
                ...
                <div className="text-right text-sm capitalize">
                  <p>{s.relationship.replaceAll("_", " ")}</p>
                  {s.isPrimaryContact && (
                    <p className="text-xs text-primary">Primary contact</p>
                  )}
                </div>
              </Link>
            ))
