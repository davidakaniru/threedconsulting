...
export function listSessionRows(
  ...
  if (f.lessonAssignmentId)
    q = q.eq("lesson_assignment_id", f.lessonAssignmentId);
  if (f.teacherId) q = q.eq("lesson_assignments.teacher_id", f.teacherId);
  if (f.programmeId) q = q.eq("lesson_assignments.programme_id", f.programmeId);
  if (f.dateFrom) q = q.gte("session_date", f.dateFrom);
  if (f.dateTo) q = q.lte("session_date", f.dateTo);
  return q;
}
...
export function insertSession(input: TablesInsert<"class_sessions">) {
  return (createAdminClient() as any)
    .from("class_sessions")
    .insert(input)
    .select(SELECT)
...
export function updateSessionRow(
  id: string,
  input: TablesUpdate<"class_sessions">,
) {
  return (createAdminClient() as any)
    .from("class_sessions")
    .update(input)
...
export function countSessions(status?: string) {
  let q = (createAdminClient() as any)
    .from("class_sessions")
...
export function teacherOwnsLessonAssignment(id: string, teacherId: string) {
  return (createAdminClient() as any)
