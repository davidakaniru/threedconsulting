...
export async function getSessions(params: any): Promise<SessionListResult> {
  const { page, pageSize, from, to } = normalizePagination(params);
  const r = await repo.listSessionRows(from, to, params);
  if (r.error)
    throw new ApiError(
  ...
  return {
    sessions: (r.data ?? []).map(mapSession),
    total: r.count ?? 0,
    page,
    pageSize,
  };
...
export async function getSession(id: string): Promise<ClassSession> {
  const r = await repo.getSessionRow(id);
  if (r.error)
    throw new ApiError(
      "SESSION_LOAD_FAILED",
...
export async function getSessionMetrics(): Promise<SessionMetrics> {
  const r = await Promise.all([
    repo.countSessions(),
    repo.countSessions("draft"),
    repo.countSessions("scheduled"),
    repo.countSessions("completed"),
    repo.countSessions("cancelled"),
  ]);
  if (r.some((x) => x.error))
    throw new ApiError(
  ...
  return {
    total: r[0].count ?? 0,
    draft: r[1].count ?? 0,
    scheduled: r[2].count ?? 0,
    completed: r[3].count ?? 0,
    cancelled: r[4].count ?? 0,
  };
}
async function assertOwnership(id: string, teacherId: string) {
  const r = await repo.teacherOwnsLessonAssignment(id, teacherId);
  if (r.error || !r.data)
    throw new ApiError(
      "FORBIDDEN",
      "You can only manage sessions for your active lessons.",
      403,
    );
}
...
export async function createSession(v: ClassSessionRequest, teacherId: string) {
  ...
  await writeAuditLog({
    ...
    metadata: {
      ...
      lessonAssignmentId: v.lessonAssignmentId,
    },
  });
  return mapSession(r.data);
}
export async function updateSession(
  ...
  if (current.lessonAssignment.teacher.id !== teacherId)
    throw new ApiError(
      "FORBIDDEN",
