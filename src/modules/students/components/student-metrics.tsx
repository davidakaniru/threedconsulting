...
export function StudentMetrics({ metrics }: { metrics: StudentMetricsType }) {
  return (
    <MetricGrid>
      ...
      <MetricCard label="Active students" value={metrics.active} icon={UserCheck} />
      <MetricCard label="Inactive students" value={metrics.inactive} icon={UserMinus} />
      <MetricCard label="Graduated" value={metrics.graduated} icon={GraduationCap} />
    </MetricGrid>
  );
}
