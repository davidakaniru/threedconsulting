...
const fields={name:yup.string().trim().required("Please enter the programme name.").max(100),description:yup.string().trim().max(1000).default(""),status:yup.string().oneOf([...PROGRAMME_STATUSES]).required()};
