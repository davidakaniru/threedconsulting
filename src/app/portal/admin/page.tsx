...
import { BookOpen, CalendarDays, FileCheck2, GraduationCap, UserRound, Users } from "lucide-react";

import { AdminPage, EmptyState, MetricCard, MetricGrid, PageHeader, QuickAction, SectionCard } from "@/components/admin/ui";
...
export default async function AdminPortalPage() {
  const [user, overview] = await Promise.all([requireAdmin(), getAdminDashboardOverview()]);
  ...
  return (
    <AdminPage>
      <PageHeader
        ...
        actions={<Button asChild><Link href="/portal/admin/enrolments"><FileCheck2 aria-hidden="true" />Review enrolments</Link></Button>}
      />
      ...
      <MetricGrid>
        ...
        <MetricCard label="Programmes" value={metrics.programmes.total} helper={`${metrics.programmes.published} published`} icon={BookOpen} tone="purple" />
      </MetricGrid>

      <SectionCard eyebrow="Shortcuts" title="Common admin actions" description="Start the tasks administrators perform most often.">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <QuickAction href="/portal/admin/programmes/new" icon={BookOpen} title="Create programme" description="Add a subject and make teachers eligible to teach it." />
          <QuickAction href="/portal/admin/enrolments" icon={FileCheck2} title="Review enrolments" description="Review parent enrolments and publish suitable opportunities to teachers." />
          <QuickAction href="/portal/admin/sessions" icon={CalendarDays} title="View sessions" description="Monitor scheduled and completed online lessons." />
        </div>
      </SectionCard>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        >
          {overview.pendingLessonRequests.length ? (
              ...
              {overview.pendingLessonRequests.map((application) => (
                <Link key={application.id} href={`/portal/admin/enrolments/${application.id}`} className="flex items-center justify-between gap-4 px-5 py-4 transition hover:bg-slate-50 sm:px-6">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 truncate text-sm text-slate-500">{application.programmeName} · Submitted by {application.parentName}</p>
                  </div>
            ...
            <div className="p-6"><EmptyState icon={FileCheck2} title="Review queue is clear" description="New parent enrolments will appear here." /></div>
          )}
        </SectionCard>
              ...
              {overview.todaySessions.map((session) => (
                <Link key={session.id} href={`/portal/admin/sessions/${session.id}`} className="flex items-start justify-between gap-4 px-5 py-4 transition hover:bg-slate-50 sm:px-6">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-slate-500">{session.programmeName} · {session.teacherName}</p>
                  </div>
